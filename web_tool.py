...
@keyframes fall {
  ...
  50% {transform: translateY(60vh) translateX(2vw) rotate(180deg);}
  100% {transform: translateY(120vh) translateX(-2vw) rotate(360deg);}
}
.blossom {
  position: fixed;
  top: -50px;
...
if not st.session_state.generated:
    ...
    for i, (ticket, message) in enumerate(st.session_state.responses):
        ...
        function copyToClipboard_{i}() {{
            ...
            const toast = document.getElementById("copied-toast-{i}");
            toast.style.display = "inline-block";
            setTimeout(() => {{ toast.style.display = "none"; }}, 2000);
        }}
        </script>
        <button class="button-copy" onclick="copyToClipboard_{i}()">📋 Copy Response {i+1}</button>
        <span id="copied-toast-{i}" class="copied-toast" style="display: none;">Copied!</span>
        """
