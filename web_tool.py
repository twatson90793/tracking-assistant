...
cols = st.columns(2)
with cols[0]:
    tickets_raw = st.text_area("🎟 Ticket Numbers (optional)", value=st.session_state.get("tickets_raw", ""), key="tickets_raw", height=400)
with cols[1]:
    trackings_raw = st.text_area("📦 Tracking Numbers (required)", value=st.session_state.get("trackings_raw", ""), key="trackings_raw", height=400)
...
# 💖 Styled Start Over Button (Fully functional)
st.markdown("""
    <style>
